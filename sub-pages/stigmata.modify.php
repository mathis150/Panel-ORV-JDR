<?php
    ...
    $effects    = $sm->getEffects($uuid);
    $conditions = $sm->getStigmaLevelupConditions($uuid);
    $histories  = $sm->getAvailableHistories();

    $validTabs = ['stig-info', 'stig-visuels', 'stig-effets', 'stig-levelup'];
    $activeTab = in_array($_GET['tab'] ?? '', $validTabs) ? $_GET['tab'] : 'stig-info';
    $fd        = $formData ?? [];

    $baseUrl = 'jdr-params?page=stigmata&sub-page=modify&uuid=' . urlencode($uuid);

    function sahv(mixed $v): string { return htmlspecialchars((string)($v ?? '')); }
    ...
    $currentRangColor = $rangColors[$s['rang']] ?? '#89CEFF';
    $actCats = StigmataManager::ACTIVATION_CATEGORIES;
?>
...
<div class="orv-menu">
    <div class="orv-menu_header">
        ...
        <span style="display:inline-block; margin-left:12px; padding:2px 10px; border-radius:12px; font-size:11px; font-weight:700;
            ...
            Rang <?php echo sahv($s['rang']); ?>
        </span>
    </div>

    <!-- Onglets -->
    <div class="session-tabs" style="padding:0 24px;">
        <button type="button" class="session-tab <?php echo $activeTab==='stig-info'    ? 'button-actif':''; ?>" data-target="stig-info">
            <i class="fa-solid fa-id-card"></i> Informations
        </button>
        <button type="button" class="session-tab <?php echo $activeTab==='stig-visuels' ? 'button-actif':''; ?>" data-target="stig-visuels">
            <i class="fa-solid fa-eye"></i> Effets visuels
        </button>
        <button type="button" class="session-tab <?php echo $activeTab==='stig-effets'  ? 'button-actif':''; ?>" data-target="stig-effets">
        ...
        <button type="button" class="session-tab <?php echo $activeTab==='stig-levelup' ? 'button-actif':''; ?>" data-target="stig-levelup">
    ...
    <div class="orv-menu_container">
        ...
        <!-- ══════════════════════════════════════════════════════
             Onglet Informations
        ══════════════════════════════════════════════════════ -->
        <div id="stig-info" class="session-tab-panel <?php echo $activeTab!=='stig-info' ? 'hidden':''; ?>">
            <form method="POST" action="<?php echo $baseUrl; ?>&tab=stig-info">
                    ...
                    <div class="orv-menu_form-elements">
                        <div class="orv-menu_form-inputs-list">
                            <div class="orv-menu_form-input" style="width:280px;">
                                ...
                                <input type="text" name="nom" value="<?php echo sahv($fd['nom'] ?? $s['nom']); ?>" required style="width:280px;">
                            </div>
                            <div class="orv-menu_form-input" style="width:180px;">
                                <label>Rang</label>
                                <select name="rang" style="width:180px;">
                                    <?php foreach (StigmataManager::RANGS as $r): ?>
                                    <option value="<?php echo $r; ?>" <?php echo ($fd['rang'] ?? $s['rang']) === $r ? 'selected' : ''; ?>>
                                        Rang <?php echo $r; ?>
                                    </option>
                                    <?php endforeach; ?>
                                </select>
                            ...
                            <div class="orv-menu_form-input" style="width:260px;">
                                ...
                                <select name="histoire_uuid" style="width:260px;">
                                    ...
                                    <option value="<?php echo sahv($h['uuid']); ?>"
                                        <?php echo ($fd['histoire_uuid'] ?? $s['histoire_uuid']) === $h['uuid'] ? 'selected' : ''; ?>>
                                        <?php echo sahv($h['titre']); ?>
                                    </option>
                                    <?php endforeach; ?>
                                </select>
                                <?php else: ?>
                                <input type="hidden" name="histoire_uuid" value="<?php echo sahv($s['histoire_uuid']); ?>">
                                <input type="text" value="<?php echo $s['histoire_uuid'] ? sahv($s['histoire_uuid']) : 'Aucune histoire disponible'; ?>" style="width:260px;" disabled>
                                <?php endif; ?>
                            </div>
                            <div class="orv-menu_form-input" style="width:160px;">
                                <label>Statut</label>
                                <label style="display:flex; align-items:center; gap:8px; cursor:pointer; margin-top:8px;">
                                    <input type="checkbox" name="is_active" value="1" <?php echo ($fd['is_active'] ?? $s['is_active']) ? 'checked' : ''; ?>>
                    ...
                    <div class="orv-menu_form-elements">
                        <div class="orv-menu_form-inputs-list">
                            <div class="orv-menu_form-input" style="width:100%; max-width:700px;">
                                ...
                                <textarea name="description" rows="6" style="width:100%;"><?php echo sahv($fd['description'] ?? $s['description']); ?></textarea>
                            </div>
                        </div>
                    </div>
                </div>

                <input type="hidden" name="effets_visuels" value="<?php echo sahv($s['effets_visuels']); ?>">
        ...
        ══════════════════════════════════════════════════════ -->
        <div id="stig-visuels" class="session-tab-panel <?php echo $activeTab!=='stig-visuels' ? 'hidden':''; ?>">
            <form method="POST" action="<?php echo $baseUrl; ?>&tab=stig-visuels">
                <input type="hidden" name="action"        value="update">
                <input type="hidden" name="nom"           value="<?php echo sahv($s['nom']); ?>">
                <input type="hidden" name="rang"          value="<?php echo sahv($s['rang']); ?>">
                <input type="hidden" name="histoire_uuid" value="<?php echo sahv($s['histoire_uuid']); ?>">
                <?php if ($s['is_active']): ?><input type="hidden" name="is_active" value="1"><?php endif; ?>
                <input type="hidden" name="description"   value="<?php echo sahv($s['description']); ?>">
                    ...
                    <div class="orv-menu_form-elements">
                        <div class="orv-menu_form-inputs-list">
                            <div class="orv-menu_form-input" style="width:100%; max-width:700px;">
                                ...
                                <textarea name="effets_visuels" rows="8" style="width:100%;"><?php echo sahv($fd['effets_visuels'] ?? $s['effets_visuels']); ?></textarea>
                            </div>
                        </div>
                    </div>
                </div>
        ...
        ══════════════════════════════════════════════════════ -->
        <div id="stig-effets" class="session-tab-panel <?php echo $activeTab!=='stig-effets' ? 'hidden':''; ?>">

            <div class="orv-menu_form">
                <div>
                    <h2 class="classic-title"><i class="fa-solid fa-wand-magic-sparkles"></i> Effets :</h2>
                    <cite>Dégâts, soins, statistiques temporaires, statuts, protections, contrôles et effets passifs de ce stigmate.</cite>
                </div>
                <form method="POST" action="<?php echo $baseUrl; ?>&tab=stig-effets">
                    ...
                    <div class="orv-menu_form-elements">
                        <div class="orv-menu_form-inputs-list" style="align-items:flex-end; flex-wrap:wrap;">
                            ...
                            <div class="orv-menu_form-input" style="width:230px;">
                                ...
                                <input type="text" name="effect_nom" placeholder="ex: Marque du Néant" style="width:230px;" required>
                            </div>
                            <div class="orv-menu_form-input" style="width:155px;">
                                <label>Catégorie</label>
                                <select name="effect_categorie" id="stig-act-categorie" style="width:155px;"
                                        onchange="syncActCat('stig')">
                            ...
                            <div class="orv-menu_form-input" style="width:170px;">
                                <label>Type</label>
                                <select name="effect_sous_type" id="stig-act-sous-type" style="width:170px;">
                                    <?php $firstCat = reset($actCats); foreach ($firstCat['sous_types'] as $stKey => $st): ?>
                                    <option value="<?php echo $stKey; ?>"><?php echo $st['label']; ?></option>
                                    <?php endforeach; ?>
                                </select>
                            </div>
                            <div class="orv-menu_form-input" style="width:190px;" id="stig-act-valeur-wrap">
                                <label id="stig-act-valeur-label"><?php echo $firstCat['valeur_label']; ?></label>
                                <input type="text" name="effect_valeur" id="stig-act-valeur"
                                       placeholder="<?php echo htmlspecialchars($firstCat['valeur_placeholder']); ?>"
                                       style="width:190px;">
                            </div>
                            <div class="orv-menu_form-input" style="width:185px;">
                                <label>Durée</label>
                                <select name="effect_type_duree" id="stig-act-type-duree" style="width:185px;"
                                        onchange="syncActDuree('stig-act-tours-wrap', this.value)">
                            ...
                            <div class="orv-menu_form-input" style="width:115px;" id="stig-act-tours-wrap">
                            ...
                            <div style="font-size:16px; font-weight:700; color:#EAEAEA; white-space:nowrap; overflow:hidden; text-overflow:ellipsis;">
                                <?php if ($eff['categorie'] === 'stat'): ?>
                                    <span style="color:<?php echo (int)$eff['valeur'] >= 0 ? '#2ECC71' : '#E74C3C'; ?>">
                                        <?php echo (int)$eff['valeur'] >= 0 ? '+' : ''; ?><?php echo sahv($eff['valeur']); ?>
                                    </span>
                                <?php else: ?>
                                    <span style="color:<?php echo $st['color']; ?>"><?php echo sahv($eff['valeur']); ?></span>
                                <?php endif; ?>
                            </div>
                        </div>
        ...
        <!-- ══════════════════════════════════════════════════════
             Onglet Level-up
        ══════════════════════════════════════════════════════ -->
        <div id="stig-levelup" class="session-tab-panel <?php echo $activeTab!=='stig-levelup' ? 'hidden':''; ?>">
            ...
            <div class="orv-menu_form">
                ...
                <form method="POST" action="<?php echo $baseUrl; ?>&tab=stig-levelup">
                    ...
                    <div class="orv-menu_form-elements">
                        <div class="orv-menu_form-inputs-list" style="align-items:flex-end;">
                            ...
                            <div class="orv-menu_form-input" style="width:160px;">
                                <label>Type</label>
                                <select name="condition_type" style="width:160px;" id="stig-levelup-type-select">
                            ...
                            <div class="orv-menu_form-input" style="width:110px;" id="stig-levelup-valeur-wrap">
                                <label>Valeur</label>
                ...
                <table>
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td style="font-size:13px;"><?php echo sahv($cond['description']); ?></td>
                            <td style="text-align:center; font-size:13px; color:rgba(137,206,255,0.7);">
                                <?php echo $cond['valeur'] !== null ? (int)$cond['valeur'] : '—'; ?>
                            </td>
                            <td>
                                <form method="POST" action="<?php echo $baseUrl; ?>&tab=stig-levelup" style="display:inline;" onsubmit="return confirm('Supprimer cette condition ?');">
...
const stigLevelupType   = document.getElementById('stig-levelup-type-select');
const stigLevelupValeur = document.getElementById('stig-levelup-valeur-wrap');
if (stigLevelupType && stigLevelupValeur) {
    stigLevelupType.addEventListener('change', () => {
        stigLevelupValeur.style.opacity = stigLevelupType.value === 'usages' ? '1' : '0.5';
    });
}
...
}, StigmataManager::ACTIVATION_CATEGORIES), JSON_UNESCAPED_UNICODE); ?>;
...
(function() {
    const sel = document.getElementById('stig-act-categorie');
    if (sel) syncActCat('stig');
})();
</script>
