<?php
    ...
    class CapacitiesManager extends Database {
        ...
        public function createCapacity(array $d): array {
            if (empty(trim($d['nom'] ?? ''))) {
                return ['success' => false, 'message' => 'Le nom de la compétence est requis.'];
            }
            $rang = in_array($d['rang'] ?? '', self::RANGS) ? $d['rang'] : 'E';
            $uuid = $this->generateCustomUUID();
            $this->makeSQLRequest(
                "INSERT INTO capacities (uuid, nom, rang, description, effets_visuels, histoire_uuid)
                 VALUES (?, ?, ?, ?, ?, ?)",
                [
                    $uuid,
                    trim($d['nom'] ?? ''),
            ...
            return ['success' => true, 'uuid' => $uuid, 'message' => 'Compétence créée.'];
        }
        ...
        public function getCapacities(): array {
            try {
                return $this->makeSQLRequest(
                    "SELECT c.uuid, c.nom, c.rang, c.is_active, c.created_at,
                            h.titre AS histoire_titre
                     FROM capacities c
                     LEFT JOIN histories h ON h.uuid = c.histoire_uuid
                     ORDER BY FIELD(c.rang,'E','D','C','B','A','S','SS','SSS'), c.nom ASC",
                    [], ORV_SQL_FETCH_ALL
                ) ?: [];
        ...
        public function getCapacityByUUID(string $uuid): ?array {
            return $this->makeSQLRequest(
                "SELECT * FROM capacities WHERE uuid = ?",
                [$uuid], ORV_SQL_FETCH_ONE
            ) ?: null;
        }

        public function getCapacityLevelupConditions(string $uuid): array {
            $rows = $this->makeSQLRequest(
                "SELECT * FROM capacity_levelup_conditions WHERE capacity_uuid = ? ORDER BY niveau_cible ASC, id ASC",
        ...
        public function updateCapacity(string $uuid, array $d): array {
            if (empty(trim($d['nom'] ?? ''))) {
                return ['success' => false, 'message' => 'Le nom de la compétence est requis.'];
            }
            $rang = in_array($d['rang'] ?? '', self::RANGS) ? $d['rang'] : 'E';
        ...
        public function deleteCapacity(string $uuid): void {
            $this->makeSQLRequest("DELETE FROM capacity_activation_effects WHERE capacity_uuid = ?", [$uuid], ORV_SQL_NOTHING);
            $this->makeSQLRequest("DELETE FROM capacity_levelup_conditions WHERE capacity_uuid = ?", [$uuid], ORV_SQL_NOTHING);
            $this->makeSQLRequest("DELETE FROM capacities                   WHERE uuid = ?",         [$uuid], ORV_SQL_NOTHING);
        }
        ...
        public function getEffects(string $uuid): array {
            return $this->makeSQLRequest(
                "SELECT * FROM capacity_activation_effects WHERE capacity_uuid = ? ORDER BY ordre ASC, id ASC",
                [$uuid], ORV_SQL_FETCH_ALL
            ) ?: [];
        }
        ...
        public function addEffect(string $uuid, string $nom, string $categorie, string $sousType, string $valeur, string $typeDuree, ?int $duree, string $description): array {
            ...
            $ordre     = $this->makeSQLRequest(
                "SELECT COALESCE(MAX(ordre), 0) + 1 AS next_ordre FROM capacity_activation_effects WHERE capacity_uuid = ?",
                [$uuid], ORV_SQL_FETCH_ONE
            )['next_ordre'] ?? 1;
            $this->makeSQLRequest(
                "INSERT INTO capacity_activation_effects
                    (capacity_uuid, nom, categorie, sous_type, valeur, type_duree, duree, description, ordre)
                 VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)",
        ...
        public function deleteEffect(int $id, string $uuid): void {
            $this->makeSQLRequest(
                "DELETE FROM capacity_activation_effects WHERE id = ? AND capacity_uuid = ?",
        ...
        public function addLevelupCondition(string $capUuid, int $niveauCible, string $type, string $description, ?int $valeur): array {
            ...
            $this->makeSQLRequest(
                "INSERT INTO capacity_levelup_conditions (capacity_uuid, niveau_cible, type, description, valeur) VALUES (?, ?, ?, ?, ?)",
                [$capUuid, $niveauCible, $type, $description, $valeur],
                ORV_SQL_NOTHING
            );
            return ['success' => true, 'message' => 'Condition ajoutée.'];
        }

        public function deleteLevelupCondition(int $id, string $capUuid): void {
            $this->makeSQLRequest(
                "DELETE FROM capacity_levelup_conditions WHERE id = ? AND capacity_uuid = ?",
                [$id, $capUuid], ORV_SQL_NOTHING
            );
        }
